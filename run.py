...
def run_train(cfg: SFTConfig):
    ...
    print("\n\033[32m===== Training w2s naive crossentropy loss model =====\033[0m")
    model_cfg, run_name = get_model_and_run_name(cfg.strong_model_name, "w2snaive")
    train_args["run_name"] = run_name
    train_args["output_dir"] = str(root / f"{weak_model_last_name}___{strong_model_last_name}" / cfg_name / "w2snaive")
    ...
    train(
        ...
        acts_dir=root / f"{weak_model_last_name}___{strong_model_last_name}" / cfg_name / "w2snaive" / "activations",
    )
